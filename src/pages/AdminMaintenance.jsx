            ...
            <AnimatePresence mode="wait">
                {activeTab === 'novo' ? (
                    ...
                    >
                        {registros.length > 0 ? (
                            registros.map(reg => (
                                <MaintenanceRecordCard key={reg.id} record={reg} />
                            ))
                        ) : (
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                    {record.fotos_antes?.map((url, i) => (
                                        <a key={i} href={url} target="_blank" rel="noreferrer" className="aspect-square rounded-2xl overflow-hidden border border-slate-200 shadow-sm hover:scale-105 transition-transform">
                                            <img src={url} className="w-full h-full object-cover" />
                                        </a>
                                    ))}
